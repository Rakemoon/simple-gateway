...
export function InteractiveBackground() {
  ...
  useEffect(() => {
    const createElements = () => {
      ...
      const types: FloatingElement["type"][] = ["dollar", "coin", "card", "banknote"]
      const colors = [
        "text-orange-300/20",
        "text-amber-300/20",
        "text-yellow-300/20",
  ...
  const calculatePosition = (element: FloatingElement) => {
    const distance = Math.sqrt(Math.pow(mousePosition.x - element.x, 2) + Math.pow(mousePosition.y - element.y, 2))
    ...
    if (distance < repulsionRadius && distance > 0) {
      ...
      const angle = Math.atan2(element.y - mousePosition.y, element.x - mousePosition.x)
        ...
        return (
          <motion.div
            ...
            animate={{
              x: position.x,
              y: position.y,
              rotate: element.rotation + Math.sin(Date.now() * 0.001 + element.id) * 10,
            }}
            transition={{
              type: "spring",
              stiffness: 150,
